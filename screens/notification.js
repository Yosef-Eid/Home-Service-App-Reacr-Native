...
export default function Notification({ route }) {
  ...
  useEffect(() => {
    if (route.params?.updatedBookings) setBookings(route.params.updatedBookings);
  ...
  const getPropertyIcon = (type) => {
    switch (type) {
      case 'Home':
        return <Feather name="home" size={20} color="#6759FF" />;
      case 'Office':
        return <FontAwesome5 name="building" size={20} color="#6759FF" />;
      case 'Vila':
        return <Ionicons name="storefront-outline" size={20} color="#6759FF" />;
      default:
        return <Feather name="home" size={20} color="#6759FF" />;
    }
  };
            ...
            <View style={{ flex: 1, justifyContent: 'center', alignItems: 'center' }}>
              <View style={{ alignItems: 'center', width: '80%', gap: 13 }}>
                ...
                <View style={{ alignItems: 'center', width: '80%', gap: 5 }}>
                  <Text className="text-2xl font-bold">No Notifications!</Text>
            ...
            bookings.map((booking, index) => (
              <TouchableOpacity
                ...
                style={[
                  ...
                  selectionMode &&
                    selectedItems.includes(index) && { borderColor: '#6759FF', borderWidth: 1 },
                ]}
                onPress={() =>
                  selectionMode ? toggleSelectItem(index) : openBookingDetails(booking)
                }
                    ...
                    {selectedBooking.location && (
                      <View style={styles.section}>
                        ...
                        <TouchableOpacity
                          ...
                          <Text style={[styles.detailText, styles.locationLink]}>View in Maps</Text>
                        </TouchableOpacity>
                        <View style={styles.detailRow}>
                          <Ionicons name="information-circle-outline" size={20} color="#6F767E" />
                          <Text style={styles.detailText}>
                            Latitude: {selectedBooking.location.latitude.toFixed(4)}, Longitude:{' '}
                            {selectedBooking.location.longitude.toFixed(4)}
                          </Text>
                        </View>
                      </View>
                    )}
